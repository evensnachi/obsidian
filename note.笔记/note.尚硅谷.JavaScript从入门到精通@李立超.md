...
语法: typeof 变量  
    - 检查字符串, 返回 string    - 检查数值时, 返回 number* */  
...
/* 如果使用 Number 表示的数字超过了最大值,  
* 则会返回一个 Infinity. 表示正无穷.  
...
/* NaN 是 Number 的数据类型, 意思是 Not a Number */a = "asd" * "gfds";  
console.log(a);  
console.log(typeof a);  
...
* 因为二进制不能精确表示 1/10* 所以千万不要使用 JS 或者其它语言的浮点数进行金融等精确计算. */  
c = 0.1 + 0.2;  
console.log(c);
```
- 